...
RKJPEGDecoder::RKJPEGDecoder(int width, int height) : JPEGDecoder(width, height) {
  ...
  if (ret != MPP_OK) {
    ...
    throw runtime_error("mpp_create failed");
  }
  ...
  if (ret != MPP_OK) {
    ...
    throw runtime_error("mpp_api_->control failed");
  }
  ...
  if (ret != MPP_OK) {
    ...
    throw runtime_error("mpp_init failed");
  }
  ...
  if (ret != MPP_OK) {
    ...
    throw runtime_error("mpp_buffer_get failed");
  }
...
bool RKJPEGDecoder::decode(const shared_ptr<ob::ColorFrame> &frame, uint8_t *dest) {
  MPP_RET ret = MPP_OK;
