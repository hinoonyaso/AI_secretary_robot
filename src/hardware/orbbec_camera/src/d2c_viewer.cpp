...
D2CViewer::D2CViewer(rclcpp::Node* const node, rmw_qos_profile_t rgb_qos,
  ...
  rgb_sub_ = make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(
      node_, "color/image_raw", rgb_qos);
  depth_sub_ = make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(
      node_, "depth/image_raw", depth_qos);
  sync_ = make_shared<message_filters::Synchronizer<MySyncPolicy>>(MySyncPolicy(10), *rgb_sub_,
                                                                        *depth_sub_);
  sync_->setMaxIntervalDuration(rclcpp::Duration::from_seconds(1.0));  // 1s

  using placeholders::_1;
  using placeholders::_2;
  sync_->registerCallback(bind(&D2CViewer::messageCallback, this, _1, _2));
  d2c_viewer_pub_ =
