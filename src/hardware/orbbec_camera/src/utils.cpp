...
void saveRGBPointsToPly(const shared_ptr<ob::Frame> &frame, const string &fileName) {
  size_t point_size = frame->dataSize() / sizeof(OBColorPoint);
  FILE *fp = fopen(fileName.c_str(), "wb+");
  shared_ptr<int> fp_guard(nullptr, [&fp](int *) {
    fflush(fp);
    fclose(fp);
  });
  fprintf(fp, "ply\n");
...
void savePointsToPly(const shared_ptr<ob::Frame> &frame, const string &fileName) {
...
ostream &operator<<(ostream &os, const OBCameraParam &rhs) {
...
string getObSDKVersion() {
  string major = to_string(ob::Version::getMajor());
  string minor = to_string(ob::Version::getMinor());
  string patch = to_string(ob::Version::getPatch());
  string version = major + "." + minor + "." + patch;
  return version;
}

OBFormat OBFormatFromString(const string &format) {
  if (format.empty()) {
    return OB_FORMAT_UNKNOWN;
  }
  string fixed_format;
  transform(format.begin(), format.end(), back_inserter(fixed_format),
                 [](const auto ch) { return isalpha(ch) ? toupper(ch) : ch; });
...
string OBFormatToString(const OBFormat &format) {
  switch (format) {
    case OB_FORMAT_MJPG:
      return "MJPG";
    case OB_FORMAT_YUYV:
...
ostream &operator<<(ostream &os, const OBFormat &rhs) {
  os << OBFormatToString(rhs);
  return os;
}

string ObDeviceTypeToString(const OBDeviceType &type) {
  switch (type) {
...
rmw_qos_profile_t getRMWQosProfileFromString(const string &str_qos) {
  string upper_str_qos = str_qos;
  transform(upper_str_qos.begin(), upper_str_qos.end(), upper_str_qos.begin(), ::toupper);
...
bool isOpenNIDevice(int pid) {
  static const vector<int> OPENNI_DEVICE_PIDS = {
  ...
  return any_of(OPENNI_DEVICE_PIDS.begin(), OPENNI_DEVICE_PIDS.end(),
                     [pid](int pid_openni) { return pid == pid_openni; });
}

OB_DEPTH_PRECISION_LEVEL depthPrecisionLevelFromString(
    const string &depth_precision_level_str) {
...
float depthPrecisionFromString(const string &depth_precision_level_str) {
  // covert 0.8mm to 0.8
  ...
  string depth_precision_level_str_num =
      depth_precision_level_str.substr(0, depth_precision_level_str.size() - 2);
  return stof(depth_precision_level_str_num);
}

OBMultiDeviceSyncMode OBSyncModeFromString(const string &mode) {
  if (mode == "FREE_RUN") {
...
OB_SAMPLE_RATE sampleRateFromString(string &sample_rate) {
  // covert to lower case
  transform(sample_rate.begin(), sample_rate.end(), sample_rate.begin(), ::tolower);
...
string sampleRateToString(const OB_SAMPLE_RATE &sample_rate) {
  switch (sample_rate) {
...
ostream &operator<<(ostream &os, const OB_SAMPLE_RATE &rhs) {
  os << sampleRateToString(rhs);
  return os;
}

OB_GYRO_FULL_SCALE_RANGE fullGyroScaleRangeFromString(string &full_scale_range) {
  transform(full_scale_range.begin(), full_scale_range.end(), full_scale_range.begin(),
                 ::tolower);
...
string fullGyroScaleRangeToString(const OB_GYRO_FULL_SCALE_RANGE &full_scale_range) {
...
ostream &operator<<(ostream &os, const OB_GYRO_FULL_SCALE_RANGE &rhs) {
  os << fullGyroScaleRangeToString(rhs);
  return os;
}

OBAccelFullScaleRange fullAccelScaleRangeFromString(string &full_scale_range) {
  transform(full_scale_range.begin(), full_scale_range.end(), full_scale_range.begin(),
                 ::tolower);
...
string fullAccelScaleRangeToString(const OBAccelFullScaleRange &full_scale_range) {
  switch (full_scale_range) {
    case OB_ACCEL_FS_2g:
      return "2g";
    case OB_ACCEL_FS_4g:
...
ostream &operator<<(ostream &os, const OBAccelFullScaleRange &rhs) {
  os << fullAccelScaleRangeToString(rhs);
  return os;
}

string parseUsbPort(const string &line) {
  string port_id;
  regex self_regex("(?:[^ ]+/usb[0-9]+[0-9./-]*/){0,1}([0-9.-]+)(:){0,1}[^ ]*",
                        regex_constants::ECMAScript);
  smatch base_match;
  bool found = regex_match(line, base_match, self_regex);
  if (found) {
    port_id = base_match[1].str();
    if (base_match[2].str().empty())  // This is libuvc string. Remove counter is exists.
    {
      regex end_regex = regex(".+(-[0-9]+$)", regex_constants::ECMAScript);
      bool found_end = regex_match(port_id, base_match, end_regex);
      if (found_end) {
...
bool isValidJPEG(const shared_ptr<ob::ColorFrame> &frame) {
...
string metaDataTypeToString(const OBFrameMetadataType &meta_data_type) {
...
ostream &operator<<(ostream &os, const OBFrameMetadataType &rhs) {
  os << metaDataTypeToString(rhs);
  return os;
}

OBHoleFillingMode holeFillingModeFromString(const string &hole_filling_mode) {
...
OBStreamType obStreamTypeFromString(const string &stream_type) {
  string upper_stream_type = stream_type;
  transform(upper_stream_type.begin(), upper_stream_type.end(), upper_stream_type.begin(),
                 ::toupper);
