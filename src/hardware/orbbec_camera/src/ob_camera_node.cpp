...
OBCameraNode::OBCameraNode(rclcpp::Node *node, shared_ptr<ob::Device> device,
  ...
  setupTopics();
#if defined(USE_RK_HW_DECODER)
  jpeg_decoder_ = make_unique<RKJPEGDecoder>(width_[COLOR], height_[COLOR]);
#elif defined(USE_NV_HW_DECODER)
  jpeg_decoder_ = make_unique<JetsonNvJPEGDecoder>(width_[COLOR], height_[COLOR]);
#endif
  if (enable_d2c_viewer_) {
    auto rgb_qos = getRMWQosProfileFromString(image_qos_[COLOR]);
    auto depth_qos = getRMWQosProfileFromString(image_qos_[DEPTH]);
    d2c_viewer_ = make_unique<D2CViewer>(node_, rgb_qos, depth_qos);
  }
...
void OBCameraNode::setAndGetNodeParameter(
    T &param, const string &param_name, const T &default_value,
    const rcl_interfaces::msg::ParameterDescriptor &parameter_descriptor) {
  try {
    param = parameters_
                ->setParam(param_name, rclcpp::ParameterValue(default_value),
                           function<void(const rclcpp::Parameter &)>(), parameter_descriptor)
                .get<T>();
...
void OBCameraNode::setupDevices() {
  ...
  try {
    ...
    if (depth_registration_) {
      align_filter_ = make_unique<ob::Align>(align_target_stream_);
    }
    if (enable_hardware_d2d_ &&
      ...
      string d2d_mode = is_hardware_d2d ? "HW D2D" : "SW D2D";
    ...
    if (!sync_mode_str_.empty() &&
      ...
      transform(sync_mode_str_.begin(), sync_mode_str_.end(), sync_mode_str_.begin(),
                     ::toupper);
    ...
    if (device_->isPropertySupported(OB_PROP_IR_AUTO_EXPOSURE_BOOL, OB_PERMISSION_WRITE)) {
      try {
        ...
        // USB2 depth-only profiles may not provide controllable IR even when property is reported.
        RCLCPP_WARN_STREAM(logger_, "Skip IR auto exposure setup: " << e.getMessage());
      }
    }
    if (ir_exposure_ != -1 &&
      ...
      try {
        ...
        RCLCPP_WARN_STREAM(logger_, "Skip disabling IR auto exposure: " << e.getMessage());
      }
    ...
    if (device_->isPropertySupported(OB_PROP_IR_LONG_EXPOSURE_BOOL, OB_PERMISSION_WRITE)) {
      try {
        ...
        RCLCPP_WARN_STREAM(logger_, "Skip IR long exposure setup: " << e.getMessage());
      }
    }
...
void OBCameraNode::printSensorProfiles(const shared_ptr<ob::Sensor> &sensor) {
  auto profiles = sensor->getStreamProfileList();
...
void OBCameraNode::setupProfiles() {
  ...
  for (const auto &elem : IMAGE_STREAMS) {
    if (enable_stream_[elem]) {
      ...
      shared_ptr<ob::VideoStreamProfile> selected_profile;
      shared_ptr<ob::VideoStreamProfile> default_profile;
      try {
...
void OBCameraNode::startStreams() {
  ...
  pipeline_ = make_unique<ob::Pipeline>(device_);
  try {
    setupPipelineConfig();
    pipeline_->start(pipeline_config_, [this](const shared_ptr<ob::FrameSet> &frame_set) {
    ...
    throw runtime_error("Failed to start pipeline");
  }
  if (enable_stream_[COLOR] && !colorFrameThread_) {
    colorFrameThread_ = make_shared<thread>([this]() { onNewColorFrameCallback(); });
  }
...
void OBCameraNode::startIMUSyncStream() {
  ...
  imuPipeline_ = make_unique<ob::Pipeline>(device_);
  ...
  shared_ptr<ob::Config> imuConfig = make_shared<ob::Config>();
  imuConfig->enableStream(accelProfile);
  imuConfig->enableStream(gyroProfile);
  imuPipeline_->enableFrameSync();
  imuPipeline_->start(imuConfig, [&](shared_ptr<ob::Frame> frame) {
    ...
    for (const auto &stream_index : HID_STREAMS) {
      if (enable_stream_[stream_index] && !imu_started_[stream_index]) {
        ...
        sensors_[stream_index]->start(
            imu_profile, [this, stream_index](const shared_ptr<ob::Frame> &frame) {
...
void OBCameraNode::getParameters() {
  setAndGetNodeParameter<string>(camera_name_, "camera_name", "camera");
  camera_link_frame_id_ = camera_name_ + "_link";
  for (auto stream_index : IMAGE_STREAMS) {
    string param_name = stream_name_[stream_index] + "_width";
    ...
    string default_frame_id = camera_name_ + "_" + stream_name_[stream_index] + "_frame";
    setAndGetNodeParameter(frame_id_[stream_index], param_name, default_frame_id);
    string default_optical_frame_id =
    ...
    setAndGetNodeParameter<string>(image_qos_[stream_index], param_name, "default");
    param_name = stream_name_[stream_index] + "_camera_info_qos";
    setAndGetNodeParameter<string>(camera_info_qos_[stream_index], param_name, "default");
  ...
  for (const auto &stream_index : HID_STREAMS) {
    string param_name = stream_name_[stream_index] + "_qos";
    setAndGetNodeParameter<string>(imu_qos_[stream_index], param_name, "default");
    param_name = "enable_" + stream_name_[stream_index];
    setAndGetNodeParameter(enable_stream_[stream_index], param_name, false);
    if (enable_sync_output_accel_gyro_) {
    ...
    setAndGetNodeParameter<string>(imu_rate_[stream_index], param_name, "");
    param_name = stream_name_[stream_index] + "_range";
    setAndGetNodeParameter<string>(imu_range_[stream_index], param_name, "");
    param_name = camera_name_ + "_" + stream_name_[stream_index] + "_frame_id";
    string default_frame_id = camera_name_ + "_" + stream_name_[stream_index] + "_frame";
    setAndGetNodeParameter(frame_id_[stream_index], param_name, default_frame_id);
    string default_optical_frame_id =
  ...
  setAndGetNodeParameter<string>(ir_info_url_, "ir_info_url", "");
  setAndGetNodeParameter<string>(color_info_url_, "color_info_url", "");
  setAndGetNodeParameter(enable_colored_point_cloud_, "enable_colored_point_cloud", false);
  setAndGetNodeParameter(enable_point_cloud_, "enable_point_cloud", false);
  setAndGetNodeParameter<string>(point_cloud_qos_, "point_cloud_qos", "default");
  ...
  setAndGetNodeParameter<string>(depth_filter_config_, "depth_filter_config", "");
  ...
  setAndGetNodeParameter<string>(depth_work_mode_, "depth_work_mode", "");
  setAndGetNodeParameter<string>(sync_mode_str_, "sync_mode", "");
  ...
  setAndGetNodeParameter<string>(depth_precision_str_, "depth_precision", "");
  ...
  setAndGetNodeParameter<string>(device_preset_, "device_preset", "");
  ...
  setAndGetNodeParameter<string>(hole_filling_filter_mode_, "hole_filling_filter_mode", "");
  ...
  setAndGetNodeParameter<string>(align_mode_, "align_mode", "HW");
  ...
  string align_target_stream_str_;
  setAndGetNodeParameter<string>(align_target_stream_str_, "align_target_stream", "COLOR");
...
void OBCameraNode::setupDiagnosticUpdater() {
  ...
  try {
    ...
    auto info = device_->getDeviceInfo();
    string serial_number = info->serialNumber();
    diagnostic_updater_ = make_unique<diagnostic_updater::Updater>(node_, diagnostic_period_);
...
void OBCameraNode::setupPipelineConfig() {
  ...
  pipeline_config_ = make_shared<ob::Config>();
  auto device_info = device_->getDeviceInfo();
  CHECK_NOTNULL(device_info.get());
  auto pid = device_info->pid();
...
void OBCameraNode::setupPublishers() {
  ...
  for (const auto &stream_index : IMAGE_STREAMS) {
    ...
    string name = stream_name_[stream_index];
    string topic = name + "/image_raw";
  ...
  if (enable_sync_output_accel_gyro_) {
    string topic_name = stream_name_[GYRO] + "_" + stream_name_[ACCEL] + "/sample";
    ...
    for (const auto &stream_index : HID_STREAMS) {
      ...
      string data_topic_name = stream_name_[stream_index] + "/sample";
...
void OBCameraNode::publishPointCloud(const shared_ptr<ob::FrameSet> &frame_set) {
  try {
...
void OBCameraNode::publishDepthPointCloud(const shared_ptr<ob::FrameSet> &frame_set) {
  (void)frame_set;
  if (!depth_cloud_pub_ || depth_cloud_pub_->get_subscription_count() == 0 ||
      !enable_point_cloud_ || !depth_frame_) {
    return;
  }
  lock_guard<decltype(point_cloud_mutex_)> point_cloud_msg_lock(point_cloud_mutex_);
  ...
  auto point_cloud_msg = make_unique<sensor_msgs::msg::PointCloud2>();
  ...
  string frame_id = depth_registration_ ? optical_frame_id_[COLOR] : optical_frame_id_[DEPTH];
  ...
  if (save_point_cloud_) {
    ...
    auto current_path = filesystem::current_path().string();
    string filename = current_path + "/point_cloud/points_" + ss.str() + ".ply";
    if (!filesystem::exists(current_path + "/point_cloud")) {
      filesystem::create_directory(current_path + "/point_cloud");
    }
  ...
  depth_cloud_pub_->publish(move(point_cloud_msg));
}

void OBCameraNode::publishColoredPointCloud(const shared_ptr<ob::FrameSet> &frame_set) {
  ...
  if (save_colored_point_cloud_) {
    ...
    ss << put_time(localtime(&now), "%Y%m%d_%H%M%S");
    auto current_path = filesystem::current_path().string();
    string filename = current_path + "/point_cloud/colored_points_" + ss.str() + ".ply";
    if (!filesystem::exists(current_path + "/point_cloud")) {
      filesystem::create_directory(current_path + "/point_cloud");
    }
    RCLCPP_INFO_STREAM(logger_, "Saving point cloud to " << filename);
  ...
  depth_registration_cloud_pub_->publish(move(point_cloud_msg));
}

shared_ptr<ob::Frame> OBCameraNode::processDepthFrameFilter(
    shared_ptr<ob::Frame> &frame) {
...
void OBCameraNode::onNewFrameSetCallback(shared_ptr<ob::FrameSet> frame_set) {
  if (!is_running_.load()) {
    return;
  }
  ...
  try {
    ...
    if (enable_stream_[COLOR] && color_frame) {
      unique_lock<mutex> lock(color_frame_queue_lock_);
    ...
    for (const auto &stream_index : IMAGE_STREAMS) {
      if (enable_stream_[stream_index]) {
        ...
        shared_ptr<ob::Frame> irFrame = decodeIRMJPGFrame(frame);
        if (irFrame) {
...
void OBCameraNode::onNewColorFrameCallback() {
  while (enable_stream_[COLOR] && rclcpp::ok() && is_running_.load()) {
    ...
    shared_ptr<ob::FrameSet> frameSet = color_frame_queue_.front();
...
shared_ptr<ob::Frame> OBCameraNode::softwareDecodeColorFrame(
    const shared_ptr<ob::Frame> &frame) {
  if (frame == nullptr) {
    return nullptr;
  }
...
bool OBCameraNode::decodeColorFrameToBuffer(const shared_ptr<ob::Frame> &frame,
                                            uint8_t *buffer) {
  if (frame == nullptr) {
    return false;
  }
...
shared_ptr<ob::Frame> OBCameraNode::decodeIRMJPGFrame(
    const shared_ptr<ob::Frame> &frame) {
  if (frame->format() == OB_FORMAT_MJPEG &&
    ...
    shared_ptr<ob::Frame> irFrame = ob::FrameHelper::createFrame(
...
void OBCameraNode::onNewFrameCallback(const shared_ptr<ob::Frame> &frame,
  ...
  shared_ptr<ob::VideoFrame> video_frame;
  ...
  image_publishers_[stream_index].publish(move(image_msg));
}

void OBCameraNode::publishMetadata(const shared_ptr<ob::Frame> &frame,
                                   const stream_index_pair &stream_index,
  ...
  for (int i = 0; i < OB_FRAME_METADATA_TYPE_COUNT; i++) {
    ...
    string field_name = metaDataTypeToString(meta_data_type);
...
void OBCameraNode::saveImageToFile(const stream_index_pair &stream_index, const cv::Mat &image,
  ...
  if (save_images_[stream_index]) {
    ...
    string file_suffix = stream_index == COLOR ? ".png" : ".raw";
    string filename = current_path + "/image/" + stream_name_[stream_index] + "_" +
                           to_string(image_msg->width) + "x" +
                           to_string(image_msg->height) + "_" + to_string(fps) + "hz_" +
                           ss.str() + "_" + to_string(index) + file_suffix;
    if (!filesystem::exists(current_path + "/image")) {
      filesystem::create_directory(current_path + "/image");
    }
...
void OBCameraNode::onNewIMUFrameSyncOutputCallback(const shared_ptr<ob::Frame> &accelframe,
                                                   const shared_ptr<ob::Frame> &gryoframe) {
...
void OBCameraNode::onNewIMUFrameCallback(const shared_ptr<ob::Frame> &frame,
                                         const stream_index_pair &stream_index) {
  if (!is_camera_node_initialized_) {
    return;
  }
...
optional<OBCameraParam> OBCameraNode::findDefaultCameraParam() {
...
optional<OBCameraParam> OBCameraNode::getDepthCameraParam() {
  auto camera_params = device_->getCalibrationCameraParamList();
  for (size_t i = 0; i < camera_params->count(); i++) {
    auto param = camera_params->getCameraParam(i);
    int depth_w = param.depthIntrinsic.width;
...
optional<OBCameraParam> OBCameraNode::getColorCameraParam() {
  auto camera_params = device_->getCalibrationCameraParamList();
  for (size_t i = 0; i < camera_params->count(); i++) {
    auto param = camera_params->getCameraParam(i);
    int color_w = param.rgbIntrinsic.width;
...
void OBCameraNode::publishStaticTF(const rclcpp::Time &t, const tf2::Vector3 &trans,
                                   const tf2::Quaternion &q, const string &from,
                                   const string &to) {
...
void OBCameraNode::publishStaticTransforms() {
  ...
  static_tf_broadcaster_ = make_shared<tf2_ros::StaticTransformBroadcaster>(node_);
  dynamic_tf_broadcaster_ = make_shared<tf2_ros::TransformBroadcaster>(node_);
  calcAndPublishStaticTransform();
  if (tf_publish_rate_ > 0) {
    tf_thread_ = make_shared<thread>([this]() { publishDynamicTransforms(); });
  } else {
...
void OBCameraNode::publishDynamicTransforms() {
  ...
  while (rclcpp::ok() && is_running_) {
    tf_cv_.wait_for(lock, chrono::milliseconds((int)(1000.0 / tf_publish_rate_)),
